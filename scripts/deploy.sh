...
PREVIOUS=$(docker ps -a --filter "name=yolo-api" --format "{{.Image}}" 2>/dev/null | head -n 1 || true)
...
if [ "$SUCCESS" = true ]; then
    ...
    NEW=$(docker ps -a --filter "name=yolo-api" --format "{{.Image}}" 2>/dev/null | head -n 1 || true)
    
    if [ -z "$NEW" ]; then
        NEW="imagem-atualizada"
    fi
